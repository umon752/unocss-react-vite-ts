...
export const useMarquee = (options: MarqueeOptions): MarqueeMethods => {
  ...
  const enableHovePause =
    options.enableHovePause ?? defaultOptions.enableHovePause;
  ...
  const reverseDirection =
    options.reverseDirection ?? defaultOptions.reverseDirection;
  ...
  const addCloneChild = (): void => {
    ...
    if (parent && child) {
      const gapStyle = parseInt(
        window.getComputedStyle(parent).getPropertyValue('gap')
      );
  ...
  const start = (): void => {
    ...
    const startMarquee = () => {
      ...
      marqueeStates.marqueeAnimationFrameId =
        requestAnimationFrame(startMarquee);
    };

    // 計算幀間隔
    if (!marqueeStates.lastFrameTime)
  ...
  const handlePosition = (moveValue: number): void => {
    if (!marqueeStates.cloneChild || marqueeStates.childArray.length === 0)
      return;
    ...
    [...marqueeStates.cloneChild].forEach((child, i) => {
      ...
      if (marqueeStates.direction === 'left') {
        marqueeStates.childArray[i].maxLeftPosition =
          marqueeStates.childPercentW * (i + 1) * -1;
        if (marqueeStates.isClick) {
          marqueeStates.childArray[i].position += -moveValue;
        } else {
          marqueeStates.childArray[i].position +=
            marqueeStates.isMouseDown || marqueeStates.isReverseDirection
              ? moveValue
              : -moveValue;
        }
        if (
          ...
          marqueeStates.childArray[i].position +=
            marqueeStates.cloneChild.length * marqueeStates.childPercentW;
          changeActive(i);
        }
      }
      // 往右
      if (marqueeStates.direction === 'right') {
        if (!marqueeStates.cloneChild) return;
        marqueeStates.childArray[i].maxRightPosition =
          (marqueeStates.cloneChild.length - (i + 1)) *
          marqueeStates.childPercentW;
        if (marqueeStates.isClick) {
          marqueeStates.childArray[i].position += moveValue;
        } else {
          marqueeStates.childArray[i].position +=
  ...
  const handleTransition = (): void => {
    ...
    if (marqueeStates.direction === 'left') {
      ...
      if (marqueeStates.isReverseDirection) {
        const diffDistance = Math.abs(
          marqueeStates.childArray[marqueeStates.activeIndex].maxLeftPosition -
            marqueeStates.childArray[marqueeStates.activeIndex].position
        );
        moveValue =
          diffDistance + marqueeStates.childPercentW + marqueeStates.gap + 2;
      }
      // active 的下一個
      else {
        const diffDistance = Math.abs(
    ...
    if (marqueeStates.direction === 'right') {
      ...
      if (marqueeStates.isReverseDirection) {
        ...
        moveValue =
          marqueeStates.childPercentW +
          (marqueeStates.childPercentW - diffDistance) +
          marqueeStates.gap +
          2;
      }
      // active 的上一個
